...
document.getElementById('noakhali-btn').addEventListener('click', function () {
    ...
    if (isNaN(noakhaliInput) || noakhaliInput <= 0) {
        return alert('Invalid entry, try again with a valid amount!')
        ...
        div.innerHTML =`
         <div class="p-8 border border-slate-300 rounded-2xl ">
           <h3 class="text-xl font-bold text-primary mb-2"> ${noakhaliInput} Taka is donated for ${noakhaliTitle} </h3>
...
document.getElementById('feni-btn').addEventListener('click', function () {
    ...
    if (isNaN(feniInput) || feniInput <= 0) {
        ...
        div.innerHTML = `
         <div class="p-8 border border-slate-300 rounded-2xl ">
           <h3 class="text-xl font-bold text-primary mb-2"> ${feniInput} Taka is donated for ${feniTitle} </h3>
           <p class="font-light text-secondary"> Date : ${new Date().toString()} </p>
        </div>
        `
    historySection.appendChild(div)
...
document.getElementById('quota-btn').addEventListener('click', function () {
    ...
    if (isNaN(quotaInput) || quotaInput <= 0) {
        ...
        div.innerHTML = `
         <div class="p-8 border border-slate-300 rounded-2xl ">
           <h3 class="text-xl font-bold text-primary mb-2"> ${quotaInput} Taka is donated for ${quotaTitle} </h3>
           <p class="font-light text-secondary"> Date : ${new Date().toString()} </p>
        </div>
        `
    historySection.appendChild(div)
...
//Donation history button working section 
